...
/**
 * Auth shell shared by every page under /auth/*. Visually mirrors the
 * user-app's AuthLayout so the brand is consistent, but the wordmark
 * gets a "Studio" caption to make the context obvious.
 */
  ...
  return (
    <div className="relative flex min-h-[100dvh] flex-col overflow-hidden bg-gradient-to-r from-[#1973FF] to-[#5048FF] text-white">
      ...
      <header className="relative flex flex-col items-center pt-8 sm:pt-10">
        <Link to="/" aria-label="LiveRush Studio home" className="flex flex-col items-center">
      ...
      <footer className="relative flex items-center justify-center px-4 pb-6 text-[11px] text-white/70 sm:pb-8 sm:text-xs">
        <span>© {new Date().getFullYear()} LiveRush Studio</span>
      </footer>
    </div>
  );
}
