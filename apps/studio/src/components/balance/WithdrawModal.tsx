...
/**
 * Creator withdrawal flow — direct port of the user-app's
 * WithdrawModal (apps/user-app/src/components/balance/WithdrawModal.tsx)
 * so creators get the exact same multi-step UX they already know from
 * being viewers. Mocked: submit is a toast, no real backend call.
 *
 * The only behavioural delta vs. the user-app version: an
 * `onWithdraw(amountCents, currency)` callback the Balance page uses
 * to flip mock commission rows from "payout" → "withdrawn" so the
 * ledger reflects the action immediately. Real payout processing
 * would replace the toast with an RPC + the callback would invalidate
 * a real query.
 */
...
interface WithdrawModalProps {
  ...
  usdCents: number;
  usdtCents: number;
  onWithdraw?: (amountCents: number, currency: Currency) => void;
}
...
export function WithdrawModal({
  ...
  // Reset to step 1 after the close animation so the next open
  // starts fresh — same UX as user-app modal.
  useEffect(() => {
    if (!open) {
...
function WithdrawFlowStep({
  ...
  // Mock "send code" — show a brief loader, then reveal the code
  // input. Same shape as user-app modal.
  ...
  // The email input is only editable in the idle state — once "Send
  // code" is pressed we lock it so the verification matches what was
  // sent.
...
function DetailRow({
  ...
  return (
    <div className="flex items-start justify-between gap-3 border-b border-border/30 py-2 first:pt-0 last:border-0 last:pb-0">
