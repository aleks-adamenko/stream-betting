...
/**
 * Hand-drawn brush-stroke button — replaces the rectangular shadcn
 * Button container with the `sidebar-button-bg.svg` path. The SVG
 * carries its own scoped linear gradient so the brand colour
 * follows the brush shape instead of a rectangular box.
 *
 * Two stock variants for now:
 *   - "primary" — blue gradient that matches `--gradient-primary`
 *     (`linear-gradient(90deg, #498aff, #493bff)`). White text.
 *   - "accent"  — yellow gradient matching the shadcn Button
 *     `accent` variant (`linear-gradient(90deg, #FFDD49, #FFBE3B)`).
 *     Dark-navy text for contrast.
 *
 * `BrushBg` is exported so callers that need bespoke hover/state
 * logic (e.g. the Follow / Following toggle on the Following page,
 * which fades between a muted neutral and a destructive overlay)
 * can stack their own SVG fills using the same path geometry.
 */

type BrushVariant = "primary" | "accent";

export interface BrushButtonProps
  extends ButtonHTMLAttributes<HTMLButtonElement> {
  variant?: BrushVariant;
  /** Optional override for height + text size. Defaults to lg (h-12 text-base). */
  size?: "default" | "lg";
  children: ReactNode;
}

export const BrushButton = forwardRef<HTMLButtonElement, BrushButtonProps>(
  function BrushButton(
    ...
    return (
      <button
        ...
        {...rest}
        className={cn(
          // Geometry mirrors the shadcn Button so callers swapping
          // from `<Button size="lg">` don't see a visual shift.
          "group relative inline-flex items-center justify-center gap-2 font-bold transition-transform hover:-translate-y-0.5 focus-visible:outline-none disabled:cursor-not-allowed disabled:opacity-60 disabled:hover:translate-y-0",
...
/**
 * Just the brush SVG with the supplied gradient fill. Use this when
 * the caller wants a custom shell (e.g. custom hover states, extra
 * stacked paths) and just needs the brush geometry + gradient
 * defs. Stretches to fill its positioned parent via
