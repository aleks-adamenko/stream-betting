...
/**
 * Auth shell shared by every page under /auth/* — same visual frame as
 * user-app + studio so the LiveRush brand is consistent across all
 * three subdomains. Wordmark gets an "Admin" caption to make the
 * context obvious; otherwise identical to studio's AuthLayout.
 */
export function AuthLayout({ children, className }: AuthLayoutProps) {
  return (
    <div className="relative flex min-h-[100dvh] flex-col overflow-hidden bg-gradient-to-r from-[#1973FF] to-[#5048FF] text-white">
      ...
      <header className="relative flex flex-col items-center pt-8 sm:pt-10">
        <Link to="/" aria-label="LiveRush Admin home" className="flex flex-col items-center">
          <img src={logoUrl} alt="LiveRush" className="h-8 w-auto sm:h-9" />
          <span className="mt-1 text-[11px] font-bold uppercase tracking-[0.3em] text-white/80">
            Admin
          </span>
        </Link>
      </header>
      ...
      <footer className="relative flex items-center justify-center px-4 pb-6 text-[11px] text-white/70 sm:pb-8 sm:text-xs">
        <span>© {new Date().getFullYear()} LiveRush Admin</span>
      </footer>
    </div>
  );
}
